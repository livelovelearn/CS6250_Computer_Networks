...
class ParkingLotTopo(Topo):
    ...
    def __init__(self, n=1, cpu=.1, bw=10, delay=None,
        ...
        # create a parking lot topology for any N>1 senders        
        s_old = s1
        for i in range(2, n+1):
            s = self.addSwitch('s' + str(i))
            h = self.addHost('h'+ str(i), **hconfig)
            self.addLink(s_old, s, port1=downlink, port2=uplink, **lconfig)
...
def run_parkinglot_expt(net, n):
    ...
    # run parkinglot_expt for any N>=1 senders
    for i in range (1, n+1):        
        sender = net.getNodeByName('h' + str(i))
        sender.sendCmd('iperf -c %s -p %s -t %d -i 1 -yc > %s/iperf_%s.txt' % (recvr.IP(), 5001, seconds, args.dir, 'h' + str(i)))
